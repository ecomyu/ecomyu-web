...
q-dialog(ref="dialog",full-width,persistent,v-model="props.params.show",@before-hide="onHide")
  q-card
        ...
        q-item.q-px-sm
          ...
          q-item-section
            q-form
              ...
              template(v-if="dialogs.post && !dialogs.post.id")
    ...
    q-card-section.row.q-pt-none
      ...
      template(v-else)
        q-btn(rounded,unelevated,,color="primary",:icon="$t('icons.post')",:label="$t('Post')",:disabled="v$.$invalid || (post.text === '' && uploadFiles.length ===0)",@click="onSubmit")
</template>
...
import { nl2br, safeText, checkUploadImageFile, getMyprofileAvatar } from 'src/utils/common'
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const load = async () => {
      let json
      if (dialogs.post.id) {
        json = await getData(`/posts/${dialogs.post.id}`)
        if (json && json._id) {
          for (let key in json) {
            post[key] = json[key]
    ...
    const onHide = () => {
      dialogs.post.id = null
      dialogs.post.show = false
    }
    ...
    const doSubmit = async () => {
      ...
      try {
        if (!dialogs.post.id) {
          json = await postData(`/posts/`, post)
        } else {
          json = await patchData(`/posts/${dialogs.post.id}`, post)
        }
      } catch (err) {
        console.log(err)
        message = 'Failed Post'
